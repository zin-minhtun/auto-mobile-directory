...
const vehicleController = {
	...
	getFeaturedVehicles: async (req, res) => {
		try {
			...
			res
				.status(500)
				.json({
					message: "Error retrieving featured vehicles",
					error: err.message,
				});
		}
	},
	...
	createVehicle: async (req, res) => {
		...
		if (req.file) {
			...
			await sharp(req.file.path).resize(300, 300).toFile(resizedImagePath);
	...
	bulkDeleteVehicles: async (req, res) => {
		try {
			...
			res
				.status(500)
				.json({ message: "Error deleting vehicles", error: err.message });
		}
	},
};
